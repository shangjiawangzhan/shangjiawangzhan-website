(()=>{
  ...
  const copy={
    en:{
      ...
      trialComplete:'Free trial complete',continueCreating:'Continue creating',trialUsed:'Your free website preview has been used. Subscribe to continue creating, updating and publishing websites.',
    ...
    zh:{
      ...
      trialComplete:'免费试用已完成',continueCreating:'继续创建',trialUsed:'你的免费网站预览机会已经使用。订阅后可以继续创建、更新和发布网站。',
  ...
  async function generateWebsite(){
    ...
    try{if(!siteId)siteId=makeSiteId();const imageIds=await uploadAll();const r=await SJW_API.generate(payloadFromForm(imageIds));previewUrl=validUrl(r.previewUrl);storage.set('sjw_site_id',siteId);let completed=false;const finish=()=>{if(completed)return;completed=true;clearPreviewTimer();done(true);refreshTrial()};preview.onload=finish;previewTimer=setTimeout(()=>{if(completed)return;completed=true;done(false);overlay.hidden=true;busy=false;$('generateBtn').disabled=false;alert(t().previewTimeout);show(create)},120000);preview.src=previewUrl}
    catch(err){clearPreviewTimer();done(false);overlay.hidden=true;show(create);if(err.code==='SIGN_IN_REQUIRED_FOR_SECOND_GENERATION'){show(create);accountDialog.showModal();text('authMessage',currentLang==='zh'?'请使用 Google 或邮箱安全链接登录，然后继续第二次免费生成。':'Continue with Google or a secure email link to use your second free generation.')}else if(err.code==='TRIAL_LIMIT'||err.code==='TRIAL_LIMIT_REACHED'||err.code==='SUBSCRIPTION_REQUIRED'||err.status===402)limit.showModal();else text('message',err.name==='AbortError'?t().generationTimeout:(err.message||t().unableCreate))}
