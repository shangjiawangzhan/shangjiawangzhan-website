(() => {
  ...
  const defaultData = () => ({
    name:'山海空间设计',city:'成都',industry:'装修设计',phone:'138 0000 0000',email:'hello@example.com',tagline:'让每一寸空间，更适合真实生活',services:'空间设计，定制橱柜，收纳规划，安装服务',about:'我们专注于清晰、耐用且适合真实生活的空间方案，从前期沟通到落地安装，持续关注每一个关键细节。'
  });
  ...
  function renderTemplates(){
    ...
    document.querySelectorAll('[data-template]').forEach(btn => btn.onclick = () => { selected = btn.dataset.template; renderTemplates(); updatePreview(); $('studio').scrollIntoView({behavior:'smooth'}); });
  }

  function current(){
    const d = defaultData();
    fields.forEach(id => d[id] = ($(id).value || d[id]).trim());
  ...
  $('generate').onclick=()=>{$('preview-wrap').scrollIntoView({behavior:'smooth'});updatePreview();};
  $('edit-site').onclick=()=>{$('studio').scrollIntoView({behavior:'smooth'});};
  $('change-template').onclick=()=>{$('templates').scrollIntoView({behavior:'smooth'});};
  ...
  $('reset').onclick=()=>{fields.forEach(id=>$(id).value='');heroImage='';logoImage='';gallery=[];selected='luxury';publicUrl='';renderTemplates();localStorage.removeItem('sjw-draft-v1');updatePreview();};
